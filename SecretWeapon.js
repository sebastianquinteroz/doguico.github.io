...
function getCountriesByType(country, type) {
  if ((type == 'PAYIN' && country.payinConfiguration != null) || (type == 'PAYOUT' && country.payinConfiguration != null) || (type == 'FX'))
    return country;
}
...
function buildOption(input, type) {
  ...
  const options = Object.values(filteredCountries).map(function(country) {
    aux += "<option value=" + country.iso + ">" + country.fullName + "</option>";
    });
...
const countriesList = {
  ...
  CR: new Country("CR", "Costa Rica", 48, "CRC", 0.13, new PayinConfiguration(["VI","VD","MC","MD"],["TU","CX"], null), new PayoutConfiguration()),
